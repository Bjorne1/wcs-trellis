 ...
 *   1. Version match between `@blulotus/trellis` and
 *      `@blulotus/trellis-core` (and the current git tag when checked from
 *      a tag context).
 ...
 *                                    on @blulotus/trellis-core resolves
...
function verifyPackedCli() {
  ...
  try {
    ...
    const dep = packedPkg.dependencies?.["@blulotus/trellis-core"];
    if (!dep) {
      fail(`packed CLI is missing dependency on @blulotus/trellis-core.`);
    }
    if (dep !== v.cliVersion) {
      fail(
        `packed CLI depends on @blulotus/trellis-core@"${dep}" but expected exact "${v.cliVersion}".\n` +
          `pnpm should rewrite workspace:* to the exact published version; got "${dep}" instead.`,
      );
    }
    console.log(
      `${GREEN}ok${RESET} packed CLI pins @blulotus/trellis-core to exact ${v.cliVersion}.`,
    );
  } finally {
