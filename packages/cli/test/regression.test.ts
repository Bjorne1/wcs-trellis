...
describe("regression: parse_simple_yaml Python execution (0.3.8)", () => {
  ...
  it("single-line flow sequences parse to lists; unclosed ones are reported", () => {
    // `spec_injection.tools: [Edit, Write]` and the `tools: []` "disable
    // everything" spelling are part of this fork's documented config surface,
    // so the parser must hand a real list to the reader instead of a string.
    const { result, stderr } = runPythonYamlFull(
      "list: [a, b]\nempty: []\nquoted: ['x', \"y\"]\ntorn: [a,\nkeep: ok\n",
    );
    expect(result).toEqual({
    ...
    expect(stderr).toContain("multi-line flow sequences are not supported");
  });
...
describe("regression: bounded index.lock retry on archive auto-commit", () => {
  ...
  function setupRepo(): void {
    ...
    // The lock contention under test only happens on the auto-commit path,
    // which this fork ships off by default (0.7.2).
    writeFile(".trellis/config.yaml", "session_auto_commit: true\n");
    writeFile(
...
describe("regression: developer identity resolution", () => {
  // Ported from #578 minus the linked-worktree inheritance step, which this
  // fork deliberately does not carry: resolution is `--assignee` >
  // TRELLIS_DEVELOPER > `.trellis/.developer`, and the "No developer set"
  // error has to name every one of them or the env var is undiscoverable.
  const pyCmd = process.platform === "win32" ? "python" : "python3";
  const pad = (n: number): string => String(n).padStart(2, "0");
  const now = new Date();
  const datePrefix = `${pad(now.getMonth() + 1)}-${pad(now.getDate())}`;
  ...
  function runTask(
    ...
    // Identity must come from the repo, never from the environment of the
  ...
  function assigneeOf(slug: string): unknown {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "trellis-dev-identity-"));
  });
  ...
  it("[developer-identity] precedence: --assignee > TRELLIS_DEVELOPER > .developer", () => {
    buildRepo("file-dev");

    expect(createTask("fromfile").status).toBe(0);
    expect(assigneeOf("fromfile")).toBe("file-dev");

    expect(
      createTask("env", [], { TRELLIS_DEVELOPER: "env-dev" }).status,
    ).toBe(0);
    expect(assigneeOf("env")).toBe("env-dev");

    expect(
      createTask("flag", ["--assignee", "flag-dev"], {
        TRELLIS_DEVELOPER: "env-dev",
      }).status,
    ).toBe(0);
    expect(assigneeOf("flag")).toBe("flag-dev");
  });

  it("[developer-identity] the env var alone is enough, with no .developer file", () => {
    buildRepo(null);
    expect(
      createTask("envonly", [], { TRELLIS_DEVELOPER: "env-dev" }).status,
    ).toBe(0);
    expect(assigneeOf("envonly")).toBe("env-dev");
  });

  it("[developer-identity] a whitespace-only env var does not count as an identity", () => {
    buildRepo("file-dev");
    expect(createTask("blank", [], { TRELLIS_DEVELOPER: "   " }).status).toBe(
      0,
    );
    expect(assigneeOf("blank")).toBe("file-dev");
  });

  it("[developer-identity] with no identity anywhere, the error names both sources", () => {
    buildRepo(null);

    const created = createTask("nobody");
    ...
    const listed = runTask(["list", "--json", "--mine"]);
  ...
  it("[developer-identity] a directory that is not a git repo fails normally, never crashes", () => {
    writeScripts(tmpDir);

    const created = createTask("nogit");
