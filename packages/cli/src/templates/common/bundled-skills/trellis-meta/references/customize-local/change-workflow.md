...
| --- | --- | --- |
| `planning` | Any of `prd.md`, `design.md`, or `implement.md` missing or incomplete | Phase 1.1: complete the planning artifacts |
| `planning` | All three artifacts complete, required sub-agent JSONL context not curated | Phase 1.3: curate the context manifests |
| `planning` | All three artifacts complete, required JSONL curated or inline mode | Phase 1.4: review artifacts, evidence for `meta.kind`, and session authorization; start only when all gates pass |
...
Phase 1.4 reuses an explicit implementation request that covers the settled scope. A planning-only request or a plan outside that authorization needs approval; routine test-seam choices do not need a separate reply.
