...
mappings by indentation, ``- `` lists of scalars, single-line flow sequences
(``key: [a, b]``, ``key: []``) parsed into lists, ``#`` comments (whole-line
...
flow mappings, multi-line flow sequences, and mappings nested inside a list —
are reported on stderr and skipped rather than parsed into a
...
def _parse_flow_sequence(value: str) -> list[str] | None:
    """Parse a single-line flow sequence (``[a, b]`` / ``[]``) into a list.

    Returned as a real list, not the raw string: a consumer that asks for a
    list must not have to re-parse YAML itself, and `packages: [a, b]` stored
    as one string reads as a single package name. ``[]`` is a meaningful empty
    list (the documented "disable everything" spelling for
    ``spec_injection.tools``), so it is distinct from an absent key.

    Returns None when the brackets do not close on this line — a multi-line
    flow sequence is left to the caller to reject.
    """
    ...
    if not body:
        return []
    return [item for item in (_unquote(part.strip()) for part in body.split(",")) if item]
...
def _unsupported_value(key: str, value: str) -> str | None:
    ...
    if value.startswith("[") and _parse_flow_sequence(value) is None:
        return "multi-line flow sequences are not supported (use `- ` list items)"
    if value.startswith("{"):
...
def _parse_yaml_block(
    ...
    while i < len(lines):
        ...
        if stripped.startswith("- "):
            ...
            if not was_quoted:
                flow = _parse_flow_sequence(value)
                if flow is not None:
                    target[key] = flow
                    i += 1
                    continue
...
def parse_simple_yaml(content: str, source: str = "config.yaml") -> dict:
    ...
    Supports:
        - key: value (string)
        - key: [a, b] / key: [] (single-line flow sequence → list)
