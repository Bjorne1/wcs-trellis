...
def cmd_deprecate(args: argparse.Namespace) -> int:
    ...
    if task_dir is None or not task_dir.is_dir():
        if task_dir is None:
            # resolve_task_dir already reported why; keep the deprecate-specific
            # refusal so it reads the same as the is_within_tasks_dir guard.
            print(colored(
                f"Error: refusing to deprecate '{task_name}': "
                f"it does not resolve to a task under {get_tasks_dir(repo_root)}",
                Colors.RED), file=sys.stderr)
        else:
            print(colored(f"Error: Task not found: {task_name}", Colors.RED), file=sys.stderr)
        return 1

    # Narrows resolve_task_dir's containment to a direct child of
    # .trellis/tasks/, enforced *before* anything is written: a mistyped name
    # must not land the banner in an unrelated prd.md.
    if not is_within_tasks_dir(task_dir, repo_root):
        print(colored(
            f"Error: refusing to deprecate '{task_name}': "
