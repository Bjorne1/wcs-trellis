...
- **Common templates**: `src/templates/common/` — single source of truth for commands, single-file workflow skills (`before-dev`, `check`, `break-loop`, `tdd`), and multi-file bundled skills. `trellis-brainstorm` and `trellis-update-spec` use bundled directories so conditional references ship with their entrypoints. Placeholders resolve per platform.
