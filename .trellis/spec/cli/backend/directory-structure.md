...
This project is a **TypeScript monorepo** using ES modules. It publishes a CLI package (`@blulotus/trellis`) and a reusable core package (`@blulotus/trellis-core`). The source code also follows a **dogfooding architecture** - Trellis uses its own configuration files (`.claude/`, `.codex/`, `.trellis/`) as templates for new projects.
...
```
packages/
├── core/                # @blulotus/trellis-core: reusable APIs
│   ├── src/
...
└── cli/                 # @blulotus/trellis: user-facing CLI
    ├── src/
...
regularly touch `.trellis/workspace/<developer>/` at the same time. Trellis
installs **no git merge attribute** for these files — it never writes to the
project-root `.gitattributes` (or anywhere else outside `.trellis/` and the
platform config dirs). Merge semantics for workspace files are the project's
own decision.

Consequences, and why both are safe:

- **`journal-N.md` conflicts are expected.** Each session appends a new session
  block, so a 3-way conflict between two parallel sessions is a pure
  append/append collision: resolve by keeping **both** sides' blocks, in any
  order. Nothing is lost and nothing needs reconciling. A project that would
  rather not see these conflicts at all can add its own rule — Trellis will not
  add or remove it:
  ...
  Note that `merge=union` is only correct while the files stay append-only; if
  a journal is ever hand-edited or reordered, a union merge silently produces
  content neither side wrote.
- **`index.md` conflicts are expected too.** It is fully rewritten every
  session (current-status counters, active-documents table, session-history
  table), so resolve by picking **either** side: `index.md` is a **derived
  summary**, not a source of truth. Real task state lives in each task's
  `task.json`. Do not put `index.md` under `merge=union` — a union merge would
  interleave two different rewrites of the same marker blocks into structurally
  broken output.

Since 0.7.2 `session_auto_commit` defaults to `false`, so Trellis does not
create workspace commits on its own and these merges only happen on commits the
project made itself. `add_session.py` prints a one-time-per-process note
(stderr, non-blocking) when it detects it is running inside a git worktree (not
the main working tree) with `session_auto_commit` explicitly enabled, pointing
